...
namespace Triggers {
	...
	public class TriggerManager {
		...
		private List<IMouseListener> fMouseListeners;

		private List<IKeyboardListener> fKeyboardListeners;
		...
		/// </remarks>
		public TriggerManager(bool InstallMouseHook, bool InstallKeyboardHook) {
		...
		public void ClearListeners() {
			...
			this.fKeyboardListeners.Clear();
		}

		public void AddTrigger(Trigger trigger) {
			trigger.TriggerManager = this;

			if ( trigger is IMouseListener ) {
				this.fMouseListeners.Add((IMouseListener)trigger);
			}
			if ( trigger is IKeyboardListener ) {
				this.fKeyboardListeners.Add((IKeyboardListener)trigger);
			}
		}
		...
		private int MouseHookProc(int nCode, int wParam, IntPtr lParam) {
			...
			if ( (nCode >= 0) && (this.fMouseListeners.Count > 0) ) {
		...
		private int KeyboardHookProc(int nCode, Int32 wParam, IntPtr lParam) {
			...
			if ( (nCode >= 0) && this.fKeyboardListeners.Count > 0 ) {
